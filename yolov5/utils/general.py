...
class ConfusionMatrix:
    ...
    def tp_fp(self):
        ...
        #tn = self.matrix.sum(0)[-1]  # true negatives
        tn = self.matrix.sum() - self.matrix.sum(0) - self.matrix.sum(1) + tp  # true negatives
        return self.matrix, tp[:-1], fp[:-1], fn[:-1], tn[:-1] # remove background class
