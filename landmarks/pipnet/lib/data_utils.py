...
def gen_target_pip(target, meanface_indices, target_map, target_local_x, target_local_y, target_nb_x, target_nb_y):
    ...
    for i in range(map_channel):                             #遍历每一个关键点
        mu_x = int(floor(target[i][0] * map_width))          #target里面的坐标是归一化之后的坐标*对应的宽高，则为输出特征图中的坐标
        mu_y = int(floor(target[i][1] * map_height))
        mu_x = max(0, mu_x)
        mu_y = max(0, mu_y)
        mu_x = min(mu_x, map_width-1)
        mu_y = min(mu_y, map_height-1)                       #判断是否越界
        target_map[i, mu_y, mu_x] = 1                        #将关键点处的坐标置为1，其它处全部为0
        shift_x = target[i][0] * map_width - mu_x            #相对于目标关键点左上角位置的具体偏移量
        shift_y = target[i][1] * map_height - mu_y
        target_local_x[i, mu_y, mu_x] = shift_x
        target_local_y[i, mu_y, mu_x] = shift_y

        for j in range(num_nb):                                             #遍历每一个临近的关键点
            nb_x = target[meanface_indices[i][j]][0] * map_width - mu_x     #通过临近关键点
            nb_y = target[meanface_indices[i][j]][1] * map_height - mu_y 
            target_nb_x[num_nb*i+j, mu_y, mu_x] = nb_x                      #第i个关键的第j个临近关键点，相对于mu_x,mu_y的距离
...
class ImageFolder_pip(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_name, target = self.imgs[index]                            #读取train.txt,labels

        img = Image.open(os.path.join(self.root, img_name)).convert('RGB')
        img, target = random_translate(img, target)                    #将标签和图片随机旋转
        img = random_occlusion(img)
        #img, target = random_flip(img, target, self.points_flip)      #将图片和标签随机翻转
        img, target = random_rotate(img, target, 30)                   #将图片和标签随机旋转30度
        img = random_blur(img)

        target_map = np.zeros((self.num_lms, int(self.input_size[1]/self.net_stride), int(self.input_size[0]/self.net_stride)))   #【N,H,W】
        target_local_x = np.zeros((self.num_lms, int(self.input_size[1]/self.net_stride), int(self.input_size[0]/self.net_stride)))
        target_local_y = np.zeros((self.num_lms, int(self.input_size[1]/self.net_stride), int(self.input_size[0]/self.net_stride)))
        target_nb_x = np.zeros((self.num_nb*self.num_lms, int(self.input_size[1]/self.net_stride), int(self.input_size[0]/self.net_stride)))
        target_nb_y = np.zeros((self.num_nb*self.num_lms, int(self.input_size[1]/self.net_stride), int(self.input_size[0]/self.net_stride)))
