...
INFO:
    ...
    LEFT_RANGE: 39                                                            
    RIGHT_RANGE: 40                                                           
    RATION: 2.65                                                                #width/height
    NUM_POINTS: 30                                                            
    SRC_ROOT: "E:/dev/gitlab/alg_for_ai/all_others/landmarks/hrnet/data/train_valid/pts2json/mpii/images"                                                                  
    DST_ROOT: "E:/dev/gitlab/alg_for_ai/all_others/landmarks/hrnet/data/train_valid/pts2json/mpii/annot"                                                     
    JSON_NAME: "train.json"                                                                                                                    
    VIS_FLAG: True                                                                                                                                        
```
* SRC_ROOT:目录中包括图像+PTS文件
* DST_ROOT中会创建一个train.json，以及一批有标记的图，json文件候选要用到
* 程序运行中会打印出w:h的值，这个值如果和config.yml中RATIO不一致，需要修改config.yml

第二步：训练[105 torch181测试通过]

参数文件：w18_672x576_adam_lr1e-3_topcrop.yaml 按照需要修改


```
python tools/train.py --cfg experiments/mpii/hrnet/w18_672x576_adam_lr1e-3_topcrop.yaml 
```
需要两个json文件
data/train_valid/mpii/annot/train.json
data/train_valid/mpii/annot/valid.json


第三步： 测试[105测试通过]

```
python tools/test.py --cfg experiments/mpii/hrnet/w18_672x576_adam_lr1e-3_topcrop_valid.yaml 
```

备注：相对训练中的yml，测试的yaml只是配置了训练出来的结果模型
