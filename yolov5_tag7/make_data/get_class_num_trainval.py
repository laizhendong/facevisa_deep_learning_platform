...
def count_num(indir):
    ...
    KeyDict = sorted(dict)
    KeyDict_with_index = [{idx:item} for idx, item in enumerate(KeyDict)]
   
    #创建data.yaml文件
    f1 = open("/code/yolov5_tag7/data.yaml","w")
    f1.writelines("path: /datasets/yolo_data/stain/images/\n")    
    f1.writelines("train: train/\n")
    f1.writelines("val: val/\n")
    f1.writelines("test: test/\n")
    f1.writelines("names: \n")  
    for i in range(len(KeyDict)):
        f1.writelines("  " + str(i) + ": " + str(KeyDict[i]) + '\n')
    f1.writelines("dataset: ['train', 'val']\n")
    f1.writelines("nc: ")
...
indir = '/datasets/voc_data/stain/imgxml'  # xml文件所在的目录
